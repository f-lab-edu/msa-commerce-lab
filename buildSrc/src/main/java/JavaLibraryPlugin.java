...
public class JavaLibraryPlugin implements Plugin<Project> {
    ...
    private void configureJava(Project project) {
        project.getExtensions().configure(JavaPluginExtension.class, java -> java.getToolchain().getLanguageVersion().set(JavaLanguageVersion.of(Version.JAVA_VERSION.getVersionAsInt())));

        project.getConfigurations().named("compileOnly").configure(compileOnly -> compileOnly.extendsFrom(project.getConfigurations().named("annotationProcessor").get()));
    }
