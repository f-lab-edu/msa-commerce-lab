...
-- Create database for Platform Services
CREATE
DATABASE IF NOT EXISTS db_platform
    CHARACTER SET = utf8mb4
    COLLATE = utf8mb4_unicode_ci;

-- Create database for Order Domain
CREATE
DATABASE IF NOT EXISTS db_order
    CHARACTER SET = utf8mb4
    COLLATE = utf8mb4_unicode_ci;

-- Create database for Payment Domain
CREATE
DATABASE IF NOT EXISTS db_payment
    CHARACTER SET = utf8mb4
    COLLATE = utf8mb4_unicode_ci;
...
CREATE TABLE db_platform.users
(
...
-- User addresses
CREATE TABLE db_platform.user_addresses
(
    id              BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (user_id) REFERENCES db_platform.users (id) ON DELETE CASCADE,
...
-- Product categories
CREATE TABLE db_platform.product_categories
(
    id          BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (parent_id) REFERENCES db_platform.product_categories (id) ON DELETE SET NULL,
...
CREATE TABLE db_platform.products
(
    id                 BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    compare_price      DECIMAL(10, 2),                        -- 할인 전 원가
    ...
    tax_class          VARCHAR(50),                           -- 세금 분류 (면세, 과세 등)
    meta_title         VARCHAR(255),                          -- SEO용 메타 제목
    meta_description   VARCHAR(500),                          -- SEO용 메타 설명
    search_keywords    TEXT,
    is_featured        BOOLEAN        NOT NULL DEFAULT FALSE, -- 추천 상품 여부
    created_at         DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at         DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (category_id) REFERENCES db_platform.product_categories (id),
    ...
    FULLTEXT           idx_products_search (name, description, search_keywords)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Product images
CREATE TABLE db_platform.product_images
(
    id         BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (product_id) REFERENCES db_platform.products (id) ON DELETE CASCADE,
...
CREATE TABLE db_platform.product_variants
(
    id                 BIGINT PRIMARY KEY AUTO_INCREMENT,
    product_id         BIGINT       NOT NULL,
    sku                VARCHAR(100) NOT NULL UNIQUE,
    ...
    price              DECIMAL(10, 2), -- 변형별 가격 (없으면 기본 상품 가격 사용)
...
CREATE TABLE db_platform.product_inventories
(
    id                   BIGINT PRIMARY KEY AUTO_INCREMENT,
    product_id           BIGINT   NOT NULL,
    product_variant_id   BIGINT NULL,
    available_quantity   INT      NOT NULL DEFAULT 0,     -- 판매 가능 재고
    reserved_quantity    INT      NOT NULL DEFAULT 0,     -- 예약된 재고 (주문 대기)
    total_quantity       INT      NOT NULL DEFAULT 0,     -- 전체 재고
    low_stock_threshold  INT      NOT NULL DEFAULT 10,    -- 재고 부족 임계값
    is_tracking_enabled  BOOLEAN  NOT NULL DEFAULT TRUE,  -- 재고 추적 여부
    is_backorder_allowed BOOLEAN  NOT NULL DEFAULT FALSE, -- 품절 시 주문 허용 여부
    last_updated_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    created_at           DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (product_id) REFERENCES db_platform.products (id) ON DELETE CASCADE,
    FOREIGN KEY (product_variant_id) REFERENCES db_platform.product_variants (id) ON DELETE CASCADE,
    UNIQUE KEY uk_inventory_product_variant (product_id, product_variant_id),
    INDEX                idx_inventory_product_id (product_id),
    INDEX                idx_inventory_available_qty (available_quantity),
...
CREATE TABLE db_platform.shopping_carts
(
    id         BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id    BIGINT   NOT NULL,
    ...
    INDEX      idx_shopping_carts_updated_at (updated_at)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Shopping cart items
CREATE TABLE db_platform.shopping_cart_items
(
    id                 BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (cart_id) REFERENCES db_platform.shopping_carts (id) ON DELETE CASCADE,
    FOREIGN KEY (product_id) REFERENCES db_platform.products (id) ON DELETE CASCADE,
    FOREIGN KEY (product_variant_id) REFERENCES db_platform.product_variants (id) ON DELETE CASCADE,
    INDEX              idx_cart_items_cart_id (cart_id),
    INDEX              idx_cart_items_product_id (product_id),
    UNIQUE KEY uk_cart_items_product (cart_id, product_id, product_variant_id)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Notifications
CREATE TABLE db_platform.notifications
(
    id               BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    type             ENUM ('EMAIL', 'SMS', 'PUSH', 'IN_APP', 'SLACK', 'WEBHOOK')                             NOT NULL,
    category         ENUM ('ORDER', 'PAYMENT', 'SHIPPING', 'PROMOTION', 'SYSTEM', 'DLT', 'ERROR', 'WARNING') NOT NULL,
    ...
    reference_type   VARCHAR(50), -- 'order', 'payment', 'dlt_message' 등
    reference_id     BIGINT,
    metadata         JSON,        -- DLT 상세 정보
    created_at       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    FOREIGN KEY (recipient_id) REFERENCES db_platform.users (id) ON DELETE CASCADE,
...
CREATE TABLE db_platform.payment_settlement_events
(
    id                     BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    payment_currency       VARCHAR(3)     NOT NULL DEFAULT 'KRW',
    ...
    gateway_fee_rate       DECIMAL(5, 4)  NOT NULL DEFAULT 0.0300, -- 3% 기본값
    gateway_fee_amount     DECIMAL(10, 2) NOT NULL DEFAULT 0.00,
    platform_fee_rate      DECIMAL(5, 4)  NOT NULL DEFAULT 0.0100, -- 1% 기본값
    platform_fee_amount    DECIMAL(10, 2) NOT NULL DEFAULT 0.00,
    tax_amount             DECIMAL(10, 2) NOT NULL DEFAULT 0.00,
    net_amount             DECIMAL(10, 2) NOT NULL,

    -- Event metadata
    event_type             ENUM ('PAYMENT_COMPLETED', 'PAYMENT_REFUNDED', 'PAYMENT_PARTIALLY_REFUNDED')                               NOT NULL,
    settlement_target_date DATE           NOT NULL,                -- 정산 대상 날짜 (결제일 기준)
    processed_at           DATETIME       NOT NULL,

    created_at             DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX                  idx_settlement_events_payment_id (payment_id),
    INDEX                  idx_settlement_events_order_id (order_id),
    INDEX                  idx_settlement_events_user_id (user_id),
    INDEX                  idx_settlement_events_target_date (settlement_target_date),
    INDEX                  idx_settlement_events_event_type (event_type),
    INDEX                  idx_settlement_events_processed_at (processed_at),
    INDEX                  idx_settlement_events_gateway (gateway_provider)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Daily Settlement Aggregation
CREATE TABLE db_platform.daily_settlements
(
    id                    BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    credit_card_amount    DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    credit_card_count     INT            NOT NULL DEFAULT 0,
    debit_card_amount     DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    ...
    digital_wallet_amount DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    digital_wallet_count  INT            NOT NULL DEFAULT 0,

    -- Gateway breakdown
    toss_amount           DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    toss_count            INT            NOT NULL DEFAULT 0,
    nice_amount           DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    nice_count            INT            NOT NULL DEFAULT 0,
    kcp_amount            DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    kcp_count             INT            NOT NULL DEFAULT 0,

    -- Financial summary
    total_gross_amount    DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    total_gateway_fee     DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    total_platform_fee    DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    total_tax_amount      DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    total_net_amount      DECIMAL(12, 2) NOT NULL DEFAULT 0.00,

    -- Processing status
    status                ENUM ('CALCULATING', 'COMPLETED', 'RECONCILED') NOT NULL DEFAULT 'CALCULATING',
    ...
    UNIQUE KEY uk_daily_settlements_date (settlement_date),
    INDEX                 idx_daily_settlements_status (status),
    INDEX                 idx_daily_settlements_calculated_at (calculated_at)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Weekly Settlement Aggregation
CREATE TABLE db_platform.weekly_settlements
(
    id                       BIGINT PRIMARY KEY AUTO_INCREMENT,
    year_week                VARCHAR(7)     NOT NULL, -- 'YYYY-WW' format (e.g., '2024-52')
    week_start_date          DATE           NOT NULL,
    week_end_date            DATE           NOT NULL,

    -- Financial summary
    total_gross_amount       DECIMAL(15, 2) NOT NULL DEFAULT 0.00,
    total_gateway_fee        DECIMAL(15, 2) NOT NULL DEFAULT 0.00,
    total_platform_fee       DECIMAL(15, 2) NOT NULL DEFAULT 0.00,
    ...
    UNIQUE KEY uk_weekly_settlements_year_week (year_week),
    INDEX                    idx_weekly_settlements_week_start (week_start_date),
    INDEX                    idx_weekly_settlements_status (status)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Monthly Settlement Aggregation
CREATE TABLE db_platform.monthly_settlements
(
    id                       BIGINT PRIMARY KEY AUTO_INCREMENT,
    settlement_year_month    VARCHAR(7)     NOT NULL, -- 'YYYY-MM' format (e.g., '2024-12')
    ...
    peak_day                 DATE NULL,
    peak_day_amount          DECIMAL(12, 2) NOT NULL DEFAULT 0.00,

    -- Processing status
    status                   ENUM ('CALCULATING', 'COMPLETED', 'RECONCILED', 'FINALIZED') NOT NULL DEFAULT 'CALCULATING',
    calculated_at            DATETIME NULL,
    reconciled_at            DATETIME NULL,
    ...
    UNIQUE KEY uk_monthly_settlements_year_month (settlement_year_month),
    INDEX                    idx_monthly_settlements_month_start (month_start_date),
    INDEX                    idx_monthly_settlements_status (status),
    INDEX                    idx_monthly_settlements_finalized_at (finalized_at)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Settlement Gateway Batches (PG사 정산 배치 정보)
CREATE TABLE db_platform.settlement_gateway_batches
(
    id                    BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    gateway_settlement_id VARCHAR(255),
    gateway_response      JSON,
    settlement_account    VARCHAR(100),

    -- Processing status
    status                ENUM ('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'RECONCILED') NOT NULL DEFAULT 'PENDING',
    ...
    INDEX                 idx_gateway_batches_provider_date (gateway_provider, settlement_date),
    INDEX                 idx_gateway_batches_status (status),
    INDEX                 idx_gateway_batches_processed_at (processed_at)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
(
    ...
    user_id                BIGINT         NOT NULL, -- Cross-domain reference (from db_platform.users)
    status                 ENUM ('PENDING', 'CONFIRMED', 'PAID', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'CANCELLED', 'REFUNDED') NOT NULL DEFAULT 'PENDING',
...
-- Order items
CREATE TABLE db_order.order_items
(
    id                   BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (order_id) REFERENCES db_order.orders (id) ON DELETE CASCADE,
...
CREATE TABLE db_order.order_status_history
(
    id                 BIGINT PRIMARY KEY AUTO_INCREMENT,
    order_id           BIGINT      NOT NULL,
...
CREATE TABLE db_platform.product_inventory_snapshots
(
    id                 BIGINT PRIMARY KEY AUTO_INCREMENT,
    product_id         BIGINT   NOT NULL,
    product_variant_id BIGINT NULL,
    ...
    snapshot_reason    ENUM ('SCHEDULED', 'MANUAL', 'DISASTER_RECOVERY') NOT NULL DEFAULT 'SCHEDULED',

    -- Note: No foreign key constraints for cross-domain references
    INDEX              idx_inventory_snapshots_product_id (product_id),
    INDEX              idx_inventory_snapshots_variant_id (product_variant_id),
    INDEX              idx_inventory_snapshots_timestamp (snapshot_timestamp),
    INDEX              idx_inventory_snapshots_reason (snapshot_reason)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- ============================================================================
-- KAFKA EVENT SOURCING TABLES (각 DB에 동일하게 존재)
-- ============================================================================

-- db_platform용
CREATE TABLE db_platform.event_store
(
    id             BIGINT PRIMARY KEY AUTO_INCREMENT,
    event_id       VARCHAR(36)  NOT NULL UNIQUE,
...
-- db_order용
CREATE TABLE db_order.event_store
(
    id             BIGINT PRIMARY KEY AUTO_INCREMENT,
    event_id       VARCHAR(36)  NOT NULL UNIQUE,
    event_type     VARCHAR(100) NOT NULL,
...
-- db_payment용
CREATE TABLE db_payment.event_store
(
    id             BIGINT PRIMARY KEY AUTO_INCREMENT,
    event_id       VARCHAR(36)  NOT NULL UNIQUE,
    event_type     VARCHAR(100) NOT NULL,
...
-- ============================================================================
-- VIEWS FOR REPORTING (Cross-domain 조회는 Application Level에서 처리)
-- ============================================================================

-- db_platform: 정산 성능 뷰 (일별 집계 기반)
CREATE VIEW db_platform.v_settlement_performance AS
SELECT ds.settlement_date,
       ds.total_gross_amount,
       ds.total_gateway_fee,
       ds.total_platform_fee,
       ds.total_net_amount,
       ds.total_payment_count,
       ds.total_refund_count,
       ROUND((ds.total_gateway_fee / NULLIF(ds.total_gross_amount, 0)) * 100, 2)  AS gateway_fee_rate,
       ROUND((ds.total_platform_fee / NULLIF(ds.total_gross_amount, 0)) * 100, 2) AS platform_fee_rate,
       CASE
           WHEN ds.total_payment_count > 0 THEN ds.total_gross_amount / ds.total_payment_count
           ELSE 0
           END                                                                    AS avg_transaction_amount
FROM db_platform.daily_settlements ds
WHERE ds.status = 'COMPLETED'
ORDER BY ds.settlement_date DESC;

-- db_platform: 월별 정산 트렌드 뷰
CREATE VIEW db_platform.v_monthly_settlement_trend AS
SELECT ms.settlement_year_month,
       ms.total_gross_amount,
       ms.total_net_amount,
       ms.total_payment_count,
       ms.avg_transaction_amount,
       ms.peak_day,
       ms.peak_day_amount,
       LAG(ms.total_gross_amount) OVER (ORDER BY ms.settlement_year_month) AS prev_month_gross, ROUND(
        ((ms.total_gross_amount - LAG(ms.total_gross_amount) OVER (ORDER BY ms.settlement_year_month))
            / NULLIF(LAG(ms.total_gross_amount) OVER(ORDER BY ms.settlement_year_month), 0)) * 100, 2
                                                                                                ) AS growth_rate_percent
FROM db_platform.monthly_settlements ms
WHERE ms.status IN ('COMPLETED', 'FINALIZED')
ORDER BY ms.settlement_year_month DESC;

-- 크로스 도메인 조회가 필요한 복잡한 리포트는
-- Application Layer에서 Kafka 이벤트 구독 + Redis 조합으로 처리