...
SET
GLOBAL innodb_default_row_format = dynamic;

-- 데이터베이스 생성
CREATE
DATABASE IF NOT EXISTS db_platform CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE
DATABASE IF NOT EXISTS db_order CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE
DATABASE IF NOT EXISTS db_payment CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE
DATABASE IF NOT EXISTS db_materialized_view CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
(
    ...
    CONSTRAINT chk_categories_slug_format CHECK
(
    slug
    REGEXP
    '^[a-z0-9]+(-[a-z0-9]+)*$'
...
(
    ...
    CONSTRAINT chk_categories_display_order CHECK
(
    display_order
    >=
    0
),
    CONSTRAINT uk_categories_parent_name UNIQUE
(
    parent_id,
    name
),
    CONSTRAINT uk_categories_parent_slug UNIQUE
(
    parent_id,
    slug
),
...
(
    ...
    INDEX idx_products_status_created
(
    status,
    created_at
    DESC
),
    INDEX idx_products_featured_created
(
    is_featured,
    status,
    created_at
    ...
    INDEX idx_products_min_order_qty
(
    min_order_quantity
),
    INDEX idx_products_max_order_qty
(
    max_order_quantity
),
    FULLTEXT
...
(
    ...
    CONSTRAINT chk_products_slug_format CHECK
(
    slug
    REGEXP
    '^[a-z0-9]+(-[a-z0-9]+)*$'
...
(
    ...
    CONSTRAINT chk_products_order_quantity CHECK
(
    max_order_quantity
    >=
    min_order_quantity
...
(
    ...
    INDEX idx_variants_sku
(
    variant_sku
),
    INDEX idx_variants_color
(
    color
),
    INDEX idx_variants_size
(
    size
),
    FOREIGN KEY
...
(
    ...
    available_quantity INT NOT NULL DEFAULT 0 CHECK
(
    available_quantity
    >=
    0
),
    reserved_quantity INT NOT NULL DEFAULT 0 CHECK
(
    reserved_quantity
    >=
    0
),
    total_quantity INT AS
(
    available_quantity
    +
    reserved_quantity
) STORED,
    low_stock_threshold INT NOT NULL DEFAULT 10 CHECK
(
    low_stock_threshold
    >=
    0
...
(
    ...
    INDEX idx_inventory_stock_status
(
    stock_status
),
...
(
    ...
    INDEX idx_inventory_events_occurred_at
(
    occurred_at
),
...
(
    ...
    INDEX idx_inventory_events_aggregate
(
    aggregate_id,
    aggregate_version
),
    INDEX idx_inventory_events_product_time
(
    product_id,
    occurred_at
),
...
(
    ...
    CONSTRAINT UNIQUE uk_inventory_agg
(
    aggregate_id,
    aggregate_version
),
    CONSTRAINT UNIQUE uk_inventory_corr
(
    correlation_id
)
    ) ENGINE = InnoDB
...
(
    ...
    INDEX idx_event_store_aggregate_id
(
    aggregate_id
),
...
(
    ...
    UNIQUE KEY uk_event_store_aggregate
(
    aggregate_id,
    sequence_number
)
...
(
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_cross_domain_events_occurred_at
(
    occurred_at
),
    INDEX idx_cross_domain_events_status
(
    publishing_status,
    retry_count
),
    INDEX idx_cross_domain_events_entity
(
    entity_type,
    entity_id
),
    INDEX idx_cross_domain_events_type
(
    event_type,
    source_domain
)
...
-- 기본 카테고리 생성
INSERT INTO product_categories (name, description, slug, display_order, is_active)
VALUES ('Electronics', '전자제품 및 IT 기기', 'electronics', 1, TRUE),
...
(
    100
) NOT NULL, -- Cross-domain reference
    user_id BIGINT NOT NULL, -- Cross-domain reference
...
(
    ...
    INDEX idx_product_sales_updated
(
    last_updated_at
),
...
(
    ...
    INDEX idx_daily_business_updated
(
    last_updated_at
)
    ) ENGINE = InnoDB
...
-- Event sourcing table for materialized views
CREATE TABLE IF NOT EXISTS event_store
(
    id
    BIGINT
...
-- 초기화 스크립트 완료 확인
INSERT
IGNORE INTO daily_business_metrics (business_date)
VALUES (CURRENT_DATE);
