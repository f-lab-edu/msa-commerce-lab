...
(
    ...
    INDEX idx_product_sales_revenue (total_revenue),
    INDEX idx_product_sales_quantity (total_quantity_sold),
    UNIQUE KEY uk_product_sales_period (product_id, summary_period, period_start_date)
) ENGINE = InnoDB
...
(
    ...
    INDEX idx_user_order_lifetime_value (lifetime_value),
    UNIQUE KEY uk_user_order_period (user_id, summary_period, period_start_date)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
(
    ...
    conversion_rate       DECIMAL(5, 2)  NULL, -- 방문자 대비 구매 전환율
    ...
    INDEX idx_daily_business_conversion_rate (conversion_rate)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
(
    ...
    domain           ENUM ('PLATFORM', 'ORDER', 'PAYMENT', 'MATERIALIZED_VIEW') NOT NULL,
    ...
    metadata         JSON,
    correlation_id   VARCHAR(36),  -- 연관 이벤트 그룹화
    causation_id     VARCHAR(36),  -- 이벤트 원인 추적
    occurred_at      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    processed_at     DATETIME     NULL,

    INDEX idx_domain_events_domain (domain),
...
CREATE TABLE IF NOT EXISTS order_summary_view (
    ...
    INDEX idx_order_summary_order_id (order_id),
    INDEX idx_order_summary_user_id (user_id),
    INDEX idx_order_summary_order_number (order_number),
    INDEX idx_order_summary_order_status (order_status),
    INDEX idx_order_summary_payment_status (payment_status),
    INDEX idx_order_summary_order_created_at (order_created_at)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS product_analytics_view (
    ...
    INDEX idx_product_analytics_product_name (product_name),
    INDEX idx_product_analytics_category_name (category_name),
    INDEX idx_product_analytics_total_orders (total_orders),
    INDEX idx_product_analytics_total_revenue (total_revenue),
    INDEX idx_product_analytics_stock_status (stock_status)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS user_activity_view (
    ...
    INDEX idx_user_activity_email (email),
    INDEX idx_user_activity_total_orders (total_orders),
    INDEX idx_user_activity_total_spent (total_spent),
    INDEX idx_user_activity_account_status (account_status),
    INDEX idx_user_activity_last_order_date (last_order_date)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS event_processing_log (
    ...
    processing_status VARCHAR(20) DEFAULT 'SUCCESS', -- SUCCESS, FAILED, RETRY
    error_message TEXT,
    retry_count INT DEFAULT 0,

    INDEX idx_event_processing_event_id (event_id),
    INDEX idx_event_processing_event_type (event_type),
    INDEX idx_event_processing_source_service (source_service),
    INDEX idx_event_processing_processed_at (processed_at),
    INDEX idx_event_processing_processing_status (processing_status)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS view_refresh_status (
    ...
    refresh_status VARCHAR(20) DEFAULT 'SUCCESS', -- SUCCESS, FAILED, IN_PROGRESS
    records_processed INT DEFAULT 0,
    error_message TEXT,
    refresh_duration_ms BIGINT,
    next_scheduled_refresh TIMESTAMP NULL,

    INDEX idx_view_refresh_view_name (view_name),
    INDEX idx_view_refresh_last_refresh_at (last_refresh_at),
    INDEX idx_view_refresh_refresh_status (refresh_status)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- Performance optimizations
ALTER TABLE product_sales_summary ADD INDEX idx_product_sales_updated (last_updated_at);
ALTER TABLE user_order_summary ADD INDEX idx_user_order_updated (last_updated_at);
ALTER TABLE daily_business_metrics ADD INDEX idx_daily_business_updated (last_updated_at);